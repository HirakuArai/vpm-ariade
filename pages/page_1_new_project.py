"""
Page 1: New Project - Charter Generation via Q&A Chat
"""
...
def show_page():
    ...
    setup_page_config("New Project", "1️⃣")
    
    st.title("1️⃣ New Project Charter")
    ...
    if not st.session_state.charter_complete:
        show_charter_chat(questions)
    else:
        show_charter_summary()
    
    # Navigation
    st.markdown("---")
    navigation_buttons("1_new_project")


def show_charter_chat(questions: list):
    """Show the Q&A chat interface"""
...
def save_charter(charter_data: dict, filename: str):
    ...
    try:
        ...
        if save_charter_data(str(charter_path), charter_data):
            ...
            st.info("You can now proceed to the next step: Preview Charter")
...
def reset_charter_session():
    """Reset charter creation session"""
    keys_to_reset = [
